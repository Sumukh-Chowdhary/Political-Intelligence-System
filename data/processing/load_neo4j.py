...
def build_knowledge_graph():
    ...
    with driver.session(database="political-intelligence") as session:
        ...
        # small mapping for state abbreviations -> full names as a final fallback
        STATE_ABBR = {
        ...
        for _, row in df_census.iterrows():
            # determine a usable state name: prefer full string, otherwise extract from NAME, then state_abbr
            state_raw = row.get('state')
            state_name = None

            if isinstance(state_raw, str) and state_raw.strip() and not state_raw.strip().isdigit():
                state_name = state_raw.strip()
            else:
                name_field = row.get('NAME') or row.get('district_name')
                if isinstance(name_field, str) and ',' in name_field:
                    state_name = name_field.split(',')[-1].strip()
                else:
                    abbr = row.get('state_abbr') or row.get('state_abbrev') or row.get('stateabbr')
                    if abbr is not None:
                        state_name = STATE_ABBR.get(str(abbr).upper(), str(abbr))
                    else:
                        state_name = str(state_raw)
